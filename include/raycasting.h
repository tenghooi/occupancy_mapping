...
void RayCasting2D(const Eigen::Vector2d& ray_origin, const Eigen::Vector2d& ray_end,
                  std::vector<Eigen::Vector3i>& output_voxels)
{
// Initialization Step
    ...
    int current_voxel_x = std::floor(ray_origin.x());
    int current_voxel_y = std::floor(ray_origin.y());
    ...
    //        tMaxX = (current_voxel_x + voxel_width - x0) / dx
    // If dx = 0, that means ray will never travel horizontally. Set tMaxX to be large number(100).
    
    double tMaxX {(dx != 0) ? (current_voxel_x + stepX - ray_origin.x()) / dx : 100};
    double tMaxY {(dy != 0) ? (current_voxel_y + stepY - ray_origin.y()) / dy : 100};
